...
if (!isset($_GET['cb'])) {
  $qs = $_GET;
  $qs['cb'] = time();
  wp_safe_redirect(home_url('/admin/login/?' . http_build_query($qs)));
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (is_wp_error($user)) {
    ...
    if ($pending) {
      wp_logout();
      wp_safe_redirect(home_url('/admin/login/?cb=' . time() . '&pending=1'));
      exit;
    }
...
<body>
  <div class="card">
    ...
    <form method="POST" autocomplete="on">
      ...
      <div class="links">
        <a href="<?php echo esc_url(home_url('/admin/register/?cb=' . time() . '&redirect=' . urlencode($redirect))); ?>">Request Access</a>
        <span>|</span>
